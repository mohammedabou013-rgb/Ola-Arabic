...
export default function GradePage() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-20 sm:pb-0">
      <Link href="/">
      ...
      <div className="text-center space-y-4 mb-12">
        <h1 className="text-4xl md:text-5xl font-black font-arabic" style={{ color: grade.color }}>
        ...
        {grade.units?.map((unit, uIdx) => (
          <div key={unit.id} className="space-y-6">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-2xl flex items-center justify-center font-black text-xl text-white shadow-sm" style={{ backgroundColor: grade.color }}>
                {uIdx + 1}
              </div>
              <h2 className="text-2xl font-black font-arabic">
                {getLocalizedText(unit.title, language)}
              </h2>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pl-4 md:pl-16 rtl:pl-0 rtl:pr-4 rtl:md:pr-16">
                    ...
                    <Card className={`group cursor-pointer border-b-4 hover:-translate-y-1 transition-all ${isLocked ? 'opacity-50 grayscale' : 'hover:border-primary'} ${isCompleted ? 'bg-primary/5' : ''}`}>
                      <CardContent className="p-5">
                        <div className="flex justify-between items-start mb-4">
                          <h3 className="text-xl font-black group-hover:text-primary transition-colors font-arabic">
                          ...
                          {isLocked ? (
                            ...
                            <div className="flex gap-1">
                              {[1,2,3].map(s => (
                                <Star key={s} className={`w-5 h-5 ${s <= stars ? 'fill-secondary text-secondary' : 'fill-muted text-muted'}`} />
                              ))}
                            </div>
                          ) : (
